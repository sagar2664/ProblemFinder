...
with open('Web_Scrapping/matrix.pkl', 'rb') as f:
   tfidf_matrix = pickle.load(f)

with open('Web_Scrapping/vectorizer.pkl', 'rb') as f:
   vectorizer = pickle.load(f)

with open("Web_Scrapping/headings.txt", 'r',encoding="utf-8") as f:
   for line in f:
      text=line.strip()
      extracted_question_name=text.split('.',1)[-1]
      QName.append(extracted_question_name)

with open("Web_Scrapping/problem_links.txt", 'r', encoding="utf-8") as f:
   for line in f:
      text=line.strip()
      QLink.append(text)
